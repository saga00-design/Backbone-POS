...
export const OfflineBanner: React.FC<OfflineBannerProps> = ({ position = 'top' }) => {
  ...
  if (showRestored) {
    if (position === 'top') {
      return (
        <div className="fixed top-0 left-0 right-0 z-[9999] bg-emerald-600 text-white py-2 text-center text-sm font-bold tracking-wide">
          Connection restored
        </div>
      );
    }
    return (
      <div className="w-full bg-emerald-600/20 border border-emerald-500/40 rounded-2xl px-4 py-3 text-center text-emerald-300 text-sm font-bold">
        Connection restored
      </div>
    );
  }
  ...
  if (position === 'top') {
    return (
      <div className="fixed top-0 left-0 right-0 z-[9999] bg-red-600 text-white flex items-center justify-center gap-3 py-3 px-4">
  ...
  return (
    <div className="w-full bg-red-500/10 border border-red-500/40 rounded-2xl p-4 flex items-start gap-3">
